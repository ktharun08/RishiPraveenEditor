...
#gurudev-panel{position:fixed;left:14px;top:105px;z-index:400;cursor:pointer;width:173px;display:flex;flex-direction:column;align-items:center;gap:6px;}
#gurudev-panel img{width:173px;border-radius:12px;border:2px solid var(--border);transition:transform .2s,box-shadow .2s;}
#gurudev-panel img:hover{transform:scale(1.04);box-shadow:0 0 20px rgba(245,166,35,.4);}
#gurudev-panel .gd-label{font-size:11px;font-weight:600;color:var(--muted);font-family:'Syne',sans-serif;text-align:center;letter-spacing:.3px;}
...
.pip-addbtn{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:9px 14px;color:var(--text);cursor:pointer;font-family:'Syne',sans-serif;font-weight:700;font-size:12px;transition:.15s;display:flex;align-items:center;gap:6px;}
...
.pip-num{width:24px;height:24px;border-radius:50%;background:var(--accent);color:#000;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;font-family:'JetBrains Mono',monospace;flex-shrink:0;}
.pip-label{font-size:13px;font-weight:700;flex:1;}
.pip-body{padding:14px 16px;display:flex;flex-direction:column;gap:10px;}
.prev-badge{display:inline-flex;align-items:center;gap:6px;background:rgba(232,255,71,.08);border:1px solid rgba(232,255,71,.25);border-radius:6px;padding:7px 12px;font-size:11px;font-family:'JetBrains Mono',monospace;color:var(--accent);}
...
<div id="p-pipeline" class="panel">
  <div style="background:rgba(232,255,71,.06);border:1px solid rgba(232,255,71,.2);border-radius:var(--radius);padding:12px 16px;margin-bottom:16px;font-size:11px;font-family:'JetBrains Mono',monospace;color:var(--muted);line-height:1.7;">
...
function pipAdd(op){
  ...
  else if(op==='merge'){   step.inputs=isFirst?(vids.length>=2?vids.slice(0,2):['','']):['__prev__',vids[1]||'']; }
...
function pipMergeDel(id,i){ var s=PIPELINE.find(function(x){return x.id===id;}); if(s){s.inputs.splice(i,1);renderPipelineSteps();} }
  ...
  el.innerHTML=PIPELINE.map(function(step,idx){
    ...
    if(step.op==='cut_multi'){
      ...
      var mergeRows=(step.inputs||[]).map(function(f,mi){
        ...
        return '<div class="merge-row"><select onchange="pipMergeSet('+id+','+mi+',this.value)">'+mopts+'</select>'+delB+'</div>';
      }).join('');
      body='<div style="font-size:11px;font-weight:700;color:var(--blue);margin-bottom:8px;font-family:\'JetBrains Mono\',monospace;">Video files to join (in order)</div>'+
...
async function runPipeline(){
  ...
  var steps=PIPELINE.map(function(step){
    ...
    if(s.op==='logo_multi')     s.windows=(_pipLogoWins[step.id]||[]).slice();
    return s;
  });
