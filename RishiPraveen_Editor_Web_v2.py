...
.sess-info{font-size:11px;color:var(--muted);font-family:'JetBrains Mono',monospace;white-space:nowrap;flex-shrink:0;}
...
@media(max-width:600px){
  .header{padding:10px 14px;}.logo{font-size:17px;}.av-logo{height:32px;}
  .panel{padding:12px;}.tabs .tab{font-size:12px;padding:10px 5px;min-width:62px;}
...
document.addEventListener('mousemove',function(e){
  if(_logoDragging){
    ...
    if(di&&fr){
      var nx=Math.max(0,Math.min(_logoDragStart.lx+(e.clientX-_logoDragStart.mx),fr.offsetWidth-di.offsetWidth));
      var ny=Math.max(0,Math.min(_logoDragStart.ly+(e.clientY-_logoDragStart.my),fr.offsetHeight-di.offsetHeight));
...
async function logoLoadPreview(){
  ...
  try{
    ...
    dragImg.onmousedown=function(e){
      _logoDragging=true;
      _logoDragStart={mx:e.clientX,my:e.clientY,
        lx:parseInt(dragImg.style.left)||0,ly:parseInt(dragImg.style.top)||0};
      e.preventDefault();
    };
